...
function ViteFeiLogo({ 
  size = 300, 
  showViteLogo = true,
  layout = 'horizontal' 
}: ViteFeiLogoProps) {
  
  const viteLogoSize = size * 0.4;
  const feiAvatarSize = size;

  const renderViteLogo = () => (
    <div className="vite-logo__container flex items-center justify-center">
      <svg 
  ...
  const getOverlayStyles = () => {
    if (layout === 'overlay') {
      return {
        vitePosition: 'absolute top-1/4 left-1/2 -translate-x-1/2 z-10',
        feiPosition: 'relative z-0'
      };
    }
    return {
      vitePosition: '',
  ...
  const { vitePosition, feiPosition } = getOverlayStyles();

  return (
    <div className={`vite-fei-logo__container ${getLayoutStyles()}`}>
      {showViteLogo && layout === 'overlay' && (
        <div className={vitePosition}>
          {renderViteLogo()}
        </div>
      )}
      <div className={feiPosition}>
        <FeiAvatar size={feiAvatarSize} />
      </div>
      {showViteLogo && layout !== 'overlay' && renderViteLogo()}
    </div>
  );
}
