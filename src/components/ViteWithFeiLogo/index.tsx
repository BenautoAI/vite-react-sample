...
interface ViteWithFeiLogoProps {
  size?: number;
  className?: string;
  layout?: 'horizontal' | 'vertical' | 'overlay';
}

function ViteWithFeiLogo(props: ViteWithFeiLogoProps) {
  const size = props.size ?? 100;
  const className = props.className ?? '';
  const layout = props.layout ?? 'horizontal';
  
  const viteSize = size;
  const feiSize = size * 0.6; // Fei avatar is slightly smaller
  ...
  const feiOverlayStyle: React.CSSProperties = {
    position: 'absolute',
    bottom: '0',
    right: '0',
  ...
  return (
    <div className={className} style={containerStyles[layout]}>
      {/* Vite Logo */}
      <svg 
      ...
      {/* Fei Avatar */}
      {layout === 'overlay' ? (
        <div style={feiOverlayStyle}>
          <FeiAvatar size={feiSize} />
        </div>
      ) : (
        <FeiAvatar size={feiSize} />
      )}
...
export default ViteWithFeiLogo;
